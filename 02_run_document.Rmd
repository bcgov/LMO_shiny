...
jo_500 <- jo_500%>%
  select(-hoo)
#interests <-  na.omit(unique(trimws(unlist(str_split(jo_500$interests, ",")))))
...
leaflet::renderLeaflet({
  bc_reg_choro(map_tbbl(), geographic_area, name, value)
})
```
...
hoo_table <- reactive({
  hoo_mod()%>%
    distinct(Geographic_Area, Typical_Education, name, get(hoo_noc()), interests, .keep_all = TRUE)%>%
...
jo_500_noc <- reactive({
  ...
  #          is.null(input[["jo_500_filters-NOC3"]]) ~ "NOC1",
  #          is.null(input[["jo_500_filters-NOC2"]]) &
...
jo_500_table <- reactive({
  jo_500_mod()%>%
    distinct(Geographic_Area, name, get(jo_500_noc()), interests, .keep_all = TRUE)%>%
