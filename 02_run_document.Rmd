...
ct <- paste(scales::comma(round(sum(pie$`Job Openings`)+1,-3)),"Job Openings \n Expected in B.C")
...
```{r}
DT::renderDT({
  DT::datatable(industry_tbbl_table())
})
```
...
occupation_outlook_table <- occupation_outlook_table%>%
  filter(name %in% c("Employment Level","Expansion Demand","Replacement Demand","Job Openings"))%>%
  pivot_wider()%>%
  rename("Job Openings {current_year+1}-{current_year+10}" := `Job Openings`,
        "Employment {current_year}" := `Employment Level`,
        "Expansion Demand {current_year+1}-{current_year+10}" := `Expansion Demand`,
