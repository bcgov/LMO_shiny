...
```{r, fig.retina=2}
pie <- occupation_outlook%>%
  filter(Geographic_Area=="British Columbia",
         name=="Job Openings",
         !is.na(NOC4))%>%
  group_by(Typical_Education)%>%
  summarize(`Job Openings`=sum(value))

#pie <- readxl::read_excel(here::here("raw_data","Education 2022E.xlsx"))
ct <- paste(scales::comma(round(sum(pie$`Job Openings`)+1, round_large)),"Job Openings \n Expected in B.C")
aest::aest_donut(pie, `Job Openings`, group_var = Typical_Education, center_text = ct, center_text_size = 2.5, lab_size=2.5)
```
...
leaflet::renderLeaflet({
  bc_reg_choro2(map_tbbl(), geographic_area, name, value)
})
```
...
hoo_noc <- reactive({
  case_when(is.null(input[["hoo_filters-NOC1"]]) &
           is.null(input[["hoo_filters-NOC2"]]) &
           is.null(input[["hoo_filters-NOC3"]]) ~ "NOC1",
           is.null(input[["hoo_filters-NOC2"]]) &
           is.null(input[["hoo_filters-NOC3"]]) ~ "NOC2",
           is.null(input[["hoo_filters-NOC3"]]) ~ "NOC3",
           TRUE~"NOC4")
})

hoo_table <- reactive({
  hoo_mod()%>%
    distinct(Geographic_Area, name, get(hoo_noc()), .keep_all = TRUE)%>%
    select(noc="get(hoo_noc())", everything())%>%
    select(-NOC1,-NOC2,-NOC3,-NOC4)%>%
    pivot_wider()
})
...
jo_500_noc <- reactive({
  case_when(is.null(input[["jo_500_filters-NOC1"]]) &
           is.null(input[["jo_500_filters-NOC2"]]) &
           is.null(input[["jo_500_filters-NOC3"]]) ~ "NOC4",
           is.null(input[["jo_500_filters-NOC2"]]) &
           is.null(input[["jo_500_filters-NOC3"]]) ~ "NOC2",
           is.null(input[["jo_500_filters-NOC3"]]) ~ "NOC3",
           TRUE~"NOC4")
})

jo_500_table <- reactive({
  jo_500_mod()%>%
    distinct(Geographic_Area, name, get(jo_500_noc()), .keep_all = TRUE)%>%
    select(noc="get(jo_500_noc())", everything())%>%
    select(-NOC1,-NOC2,-NOC3,-NOC4)%>%
    pivot_wider()
})
